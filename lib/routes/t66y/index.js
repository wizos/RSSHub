...
module.exports = async (ctx) => {
    ...
    if (routeParams) {
        ...
        size = fallback(undefined, queryToInteger(parsed.size), 25);
    ...
    list = $('.tr3', list).not('.tr2.tac').nextAll().slice(0, size).get();
    ...
    const parseContent = (htmlString) => {
        ...
        for (let k = 0; k < images.length; k++) {
	        let oriSrc = $(images[k]).attr('ess-data');
	        if(oriSrc){
		        $(images[k]).replaceWith(`<img src="${oriSrc}" />`);
	        }
        }
        // Handle input tag
        images = $('input');
