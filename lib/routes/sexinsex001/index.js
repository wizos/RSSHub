...
module.exports = async (ctx) => {
    ...
    const fetch = async (pageindex) => {
	    ...
	    //console.log("sis001获取的列表资源1：" + res.data);
	    //logger.info("sis001获取的列表资源2：" + res.data);
	    //throw Error("sis001获取的列表资源3：" + res.data);
        
        const $ = cheerio.load(res.data);
        title = (ctx.params.type ? `[${$('#headfilter > ul > li.current').text()}]` : '') + $('title').text();
        let list = $('tbody[id^=stickthread_] > tr, tbody[id^=normalthread_] > tr').get();
        list = list.filter(function () {
	        const els = $(this).find('a[href$="typeid=528"], #thread_11043424, #thread_10998096');
        ...
        const result = await Promise.all(
            list.slice(0, 20).map(async (item) => {
                ...
                const guid = $('span[id^=thread]').attr('id');
