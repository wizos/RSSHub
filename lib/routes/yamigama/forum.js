...
module.exports = async (ctx) => {
    ...
    const base = 'https://www.yamigama.com';
    ...
    const out = await Promise.all(
        list.slice(0, 20).map(async (item) => {
            ...
            const title = $('table:nth-child(1) > tbody > tr > td:nth-child(1) > font:nth-child(1)').text();
            const authorObj = $('table:nth-child(1) > tbody > tr > td:nth-child(1) > b');
            const author = authorObj.text();
            const authorHtml = authorObj.html();
            ...
            //const entryInfoList = $('table[cellpadding="2"]');
            //entryInfoList.each((i, e) => {
