...
module.exports = async (ctx) => {
    ...
    const items = await Promise.all(
        objects.map(async (item) => {
            const original_price = parseFloat(Number(item.discount_info[0].original_price).toFixed(2));
            const discointed_price = parseFloat(Number(item.discount_info[0].discounted_price).toFixed(2));
            return {
