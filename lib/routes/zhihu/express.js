...
module.exports = async (ctx) => {
    ...
    const out = await Promise.all(
        list.filter((el) => {return (el.verb === 'ANSWER_CREATE' || el.verb === 'MEMBER_CREATE_ARTICLE' || el.verb === 'MEMBER_CREATE_PIN') } )
        .map((item) => {
            ...
            return {
                title: `${list[0].actor.name}${item.action_text}: ${title}`,
                author: author,
    ...
    ctx.state.data = {
        title: `${list[0].actor.name}的知乎发表`,
        link: `https://www.zhihu.com/people/${id}/activities`,
        description: list[0].actor.headline || list[0].actor.description,
