...
module.exports = (html, timeZone = -serverOffset) => {
    let math;
    let date = new Date();
    if (/(\d+)\s*分钟前/.exec(html)) {
        math = /(\d+)\s*分钟前/.exec(html);
        date.setMinutes(date.getMinutes() - math[1]);
        date.setSeconds(0);
    } else if (/(\d+)\s*小时前/.exec(html)) {
        math = /(\d+)\s*小时前/.exec(html);
        date.setHours(date.getHours() - math[1]);
    } else if (/(\d+)\s*天前/.exec(html)) {
        math = /(\d+)\s*天前/.exec(html);
        date.setDate(date.getDate() - math[1]);
    } else if (/(\d+)\s*月前/.exec(html)) {
        math = /(\d+)\s*月前/.exec(html);
        date.setMonth(date.getMonth() - math[1]);
    } else if (/(\d+)\s*年前/.exec(html)) {
        math = /(\d+)\s*年前/.exec(html);
